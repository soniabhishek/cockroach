...
func (pg *postgres_db) SelectById(holder interface{}, id string) error {

	ptrv := reflect.ValueOf(holder)
	if ptrv.Kind() != reflect.Ptr {
		e := fmt.Sprintf("gorp: passed non-pointer: %v (kind=%v)", holder,
			ptrv.Kind())
		return errors.New(e)
	}
	elem := ptrv.Elem()
	...
	return pg.gorpDbMap.SelectOne(holder, queries.SelectById(table.TableName), id)
}
