  ...
  SELECT CASE
    WHEN nullable THEN
      CASE typ
        ...
        WHEN 'date'                        THEN 'pq.NullTime'
        WHEN 'time with time zone'         THEN 'pq.NullTime'
        WHEN 'time without time zone'      THEN 'pq.NullTime'
...
WITH structs AS (
  ...
  SELECT (get_go_name(regexp_replace(table_name, '([^aeiou])s$', '\1'))) AS type_identifier,
         string_agg(E'\t' || name_pg2go(column_name, true) || ' '
                          ...
                          || ' `db:"' || column_name || '"'
                          || ' json:"'|| column_name || '"'
                          || ' bson:"'|| (CASE WHEN column_name = 'id' THEN '_id' ELSE column_name END) || '"`'
